...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://smartaitoolslist.com/${article.slug}` },
  }
}
...
export default function ArticlePage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    author: { '@type': 'Organization', name: 'SmartAIToolsList' },
    publisher: { '@type': 'Organization', name: 'SmartAIToolsList' },
  }
    ...
    <>
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <header className="mb-10">
          <span className="text-xs font-medium text-purple-700 bg-purple-50 px-2 py-0.5 rounded mb-3 inline-block">
            {article.category}
          </span>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400 border-t border-b border-gray-100 py-3">
            <span>By <strong className="text-gray-600">SmartAIToolsList Editorial</strong></span>
            <span>
              Updated:{' '}
              {new Date(article.updatedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
            </span>
            <span>{article.readingTime} min read</span>
            {article.verified && <span className="badge-verified">✓ Hands-On Tested</span>}
          </div>
        </header>
        ...
        {/* CTA */}
        <section className="rounded-2xl bg-black text-white p-8 text-center">
          <p className="text-xs uppercase tracking-widest text-purple-400 mb-2">Our Top Pick</p>
          <h3 className="text-xl font-serif font-bold mb-2">{article.ctaTitle}</h3>
          <p className="text-sm text-gray-300 mb-6 max-w-md mx-auto leading-relaxed">{article.ctaDescription}</p>
          <a
            href={article.ctaAffiliateUrl}
            target="_blank"
            rel="nofollow noopener noreferrer"
            className="inline-block bg-purple-600 text-white font-semibold px-6 py-3 rounded-xl hover:bg-purple-700 transition-colors text-sm"
          >
